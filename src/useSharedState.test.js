...
class ErrorBoundary extends React.Component<{ children: Node, onError: (Error) => void }, { hasError: boolean }> {
	...
	render() {
		return (
			this.state.hasError ? <div /> : this.props.children
		);
	}
...
test( 'react-sharedb: useSharedReducer works with <Suspense> and component lifecycles to manage the lifetime of sharedb docs.', async assert => {
...

	{
		...
		assert.ok(
			mock_instance,
			'<Mock> renders after the share doc was created.'
		);
		...
		<SharedStateProvider connection={ local } >
			<Suspense fallback={ <Loading /> } >
				<SharedState key={ doc_id } collection={ collection } docId={ doc_id } >
					{ (state, submit) => 
						<Mock count={ state.count } onSubmit={ submit } /> 
					}
				</SharedState>
			</Suspense>
		</SharedStateProvider>
...
test.skip('react-sharedb: useSharedReducer suspends rendering of an already rendered component when the doc id changes.', async assert => {
			...
			<SharedStateProvider connection={ local } >
				<Suspense fallback={ <Loading /> } >
					<SharedState collection={ collection } docId={ doc_id_0 } >
						{ (state, submit) => 
							<Mock state={ state } onSubmit={ submit } /> 
						}
					</SharedState>
				</Suspense>
			</SharedStateProvider>
...

	{
		...
		assert.deepEquals( mock.props.state, doc_0_state, 'and the state prop is equal to the state of the first doc.' );
	}
...

	{
		...
		assert.notOk( !!mock, 'After updating <SharedState> with the second doc id the rendering of <Mock> was suspended' );
		//assert.deepEquals( mock_instance.props.state, doc_0_state, 'and the state prop is equal to the state of the first doc.' );
		assert.ok( !!loading, 'and <Suspense> has rendered <Loading> in it\'s place.' );
	}
...
test.skip('react-sharedb: useSharedReducer suspends rendering if a document is deleted while a component is subscribed to it.', async assert => {
	...
	try {
...

		{
			...
			assert.ok( !!loading_instance, 'and <Suspense> has fallen back to <Loading> in place of <Mock>.' );
		}
