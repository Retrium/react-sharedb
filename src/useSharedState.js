...
export function useSharedState(collection: string, doc_id: string): [any, (mixed) => Promise<void>] {

	const connection = useContext(ShareContext);

	if( !connection ) {
		throw new Error('The useSharedReducer hook must be mounted under a <SharedStateProvider>. Add a <SharedStateProvider> further up the react tree.');
	}
